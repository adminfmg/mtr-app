...
// Supabase service key (bypass RLS untuk insert)
...
export async function POST(req: Request) {
  try {
    ...
    // 2. Kirim email notifikasi via Mailjet
    await mailjet.post('send', { version: 'v3.1' }).request({
      Messages: [
        {
          From: {
            Email: 'noreply@mytradingreviews.com',
            Name: 'MTR Notifications',
          },
          To: [
            {
              Email: 'contact@mytradingreviews.com',
              Name: 'MTR Team',
            },
          ],
          ReplyTo: {
